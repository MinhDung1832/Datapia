...
namespace Datapia.Controllers
{
    public class AccountController : Controller
    {
        ...
        public ActionResult Register(FormCollection ip_form_1)
        {
            ...
            {
                //var check = SendEmail(emailaddress,"Mã xác thực tài khoản",DataAccess.Common.RandomString(6));
                // if (check)
                // {
                // }
                // @Session["err"] = "Đăng ký không thành công, vui lòng thử lại sau!";
                // return View();
                var regis = BaseConnectionSql.Execute_Update_Insert_V1("strConn", "sp_user_register", user_code, user_name, Common.MD5_encript(password), emailaddress);
                if (regis)
                {
        ...
        //        email.From.Add(new MailboxAddress("Sender Name", "mailtrap@demomailtrap.com"));
        ...
        //        {
        //            smtp.Connect("live.smtp.mailtrap.io", 587, false);

        //            // Note: only needed if the SMTP server requires authentication
        //            smtp.Authenticate("api", "36504961fd0ae808dd5e9ff1818d0dee");
